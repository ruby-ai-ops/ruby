---
name: ruby-llm
description: Step-by-step guide for adding support for a new LLM in Ruby, and for deprecating or removing the model it supersedes (including the agent-config repoint migration). Use when adding a new model, updating a previous one, or retiring a decommissioned one.
---
...
1. **Model config** (`front/types/assistant/models/*`) — the legacy `ModelConfigurationType`
   ...
   of truth consumed by the UI, pricing, and the ruby layer.
2. **`model_constructors`** (`front/lib/model_constructors/*`) — provider-agnostic endpoint
   **classes**, one per `(provider, model, region, provider-api)`. Each class mixes a shared
   ...
   config live.
3. **`llms` (ruby layer)** (`front/lib/llms/*`) — thin Ruby-specific wrappers around the
   `model_constructors` classes that add Ruby concerns (display name, `byok`, endpoint
   filters, and any **caps** — e.g. exposing 250k context on a model that natively supports
   1M). Registered into `RUBY_STREAM_ENDPOINTS`.
...
### E. `llms` — the ruby layer (stream)

| File | What to add |
|------|-------------|
| `front/lib/llms/providers/{provider}/models/{model}.ts` | **Ruby config mixin** `WithRubyXConfig(Base)` — `Object.assign`es the legacy `X_MODEL_CONFIG` onto the class and overrides `displayName`/`description`/`byok` (and any caps). |
| `front/lib/llms/stream/endpoints/{...}.ts` | One thin ruby wrapper per endpoint extending the `model_constructors` class via the ruby mixin; call `defineRubyStreamEndpoint(...)`. |
| `front/lib/llms/stream/index.ts` | Register each **available** ruby endpoint in `RUBY_STREAM_ENDPOINTS` (`satisfies Record<StreamEndpointId, ...>`). |

### F. SDK + UI

| File | What to add |
|------|-------------|
| `sdks/js/src/types.ts` | Add the id to the `KnownModelLLMId` union. **Then rebuild the SDK types** (`cd sdks/js && npm run build:types`) — `front`'s `sdk_drift.test.ts` type-imports the built `@ruby-ai/client`, so `tsgo` reads stale declarations until you do. |
...
describes what the provider accepts — not what Ruby happens to send today, and not what would
...
field is `z.literal(1).optional().default(1)` — not `z.undefined()`, even though the Ruby layer
...
and why Ruby doesn't, so the next reader doesn't mistake it for a provider constraint.
...
```bash
cd front
NODE_ENV=test RUN_LLM_TEST=true RUBY_MANAGED_{PROVIDER}_API_KEY=... \
  npm run test -- --config lib/model_constructors/test/vite.config.js \
  lib/model_constructors/test/endpoints/{...}.test.ts
```

Env-var names live in the sibling's `createInstance` (`RUBY_MANAGED_ANTHROPIC_API_KEY`,
`RUBY_MANAGED_GOOGLE_AI_STUDIO_API_KEY`, …). Agent-platform/Vertex endpoints need
...
- `sdk_drift.test.ts` is a **compile-time** guard: its `it()` body always passes, and the
  ...
  first, or `tsgo` reads a stale `@ruby-ai/client` and passes on a drifted id.
...
| `front/lib/api/assistant/global_agents/configurations/ruby/ruby.ts` | Repoint every `preferredModelConfiguration` naming it (e.g. the `ruby-kimi*` family). |
...
| `front/lib/api/assistant/global_agents/global_agents.ts` | If the old model had its **own** global agent (rather than a `ruby-*` agent you just repointed), add its `GLOBAL_AGENTS_SID` to `RETIRED_GLOBAL_AGENTS_SID`. |
...
- **SDK**: removing an id from `KnownModelLLMId` in `sdks/js/src/types.ts` **narrows a
  ...
  `ruby-breaking-changes` skill), then rebuild (`cd sdks/js && npm run build:types`).
...
| Property | Notes |
|----------|-------|
| `contextSize` / `generationTokensCount` | Real provider values (legacy config). Caps go in the ruby layer. |
...
- [ ] `llms` ruby layer: ruby mixin + endpoint(s) + `llms/stream/index.ts`
...
- [ ] `MODEL_STREAMS` candidates and `ruby-*` global agents repointed to the new model,
...
- **`tsgo` fails on `sdk_drift.test.ts` naming your id** → add it to `KnownModelLLMId` in `sdks/js/src/types.ts`, then `cd sdks/js && npm run build:types` (it type-imports the built `@ruby-ai/client` declarations, so the rebuild must come first).
...
- **Live suite 401s** → check the key you actually exported. A shell profile can define the same `RUBY_MANAGED_*_API_KEY` twice; the last export wins interactively, so grepping for the first match can hand you a stale key.
