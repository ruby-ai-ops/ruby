...
const CliChat: FC<CliChatProps> = ({
  ...
  const createConversationForFiles = useCallback(
    async (title: string) => {
      ...
      const convRes = await rubyClient.createConversation({
        title,
        visibility: "unlisted",
        contentFragments: [],
  ...
  const handleConversationSelected = useCallback(
    async (convId: string) => {
      ...
      const convRes = await rubyClient.getConversation({
  ...
  // Auto-select @ruby agent when no agent/sId/search is specified
  useEffect(() => {
    ...
    const rubyAgent = allAgents.find((agent) => agent.sId === "ruby");
    if (rubyAgent) {
      setSelectedAgent(rubyAgent);
      setConversationItems([
        {
          key: "welcome_header",
          type: "welcome_header",
          agentName: rubyAgent.name,
          agentDescription: rubyAgent.description,
        },
      ]);
    }
      ...
      try {
        ...
        if (attachedFiles.length > 0 && currentConversationId) {
          for (const file of attachedFiles) {
            const fragmentRes = await rubyClient.postContentFragment({
        ...
        if (!currentConversationId) {
          ...
          const messageRes = await rubyClient.postUserMessage({
            conversationId: currentConversationId,
            message: {
              content: questionText,
              mentions: [{ configurationId: selectedAgent.sId }],
        ...
        const streamRes = await rubyClient.streamAgentAnswerEvents({
        ...
        for await (const event of streamRes.value.eventStream) {
          if (event.type === "generation_tokens") {
            ...
            await rubyClient.validateAction({
    ...
    if (key.ctrl && input === "g") {
      if (currentConversationId) {
        void (async () => {
          const workspaceId = await AuthService.getSelectedWorkspaceId();
          if (workspaceId) {
            const url = `https://ruby.ad/w/${workspaceId}/agent/${currentConversationId}`;
            await open(url);
          } else {
