...
1. Front returns a short-lived signed GCS download URL for the node's current
   blob.
2. `dsbx` downloads it into `/run/ruby-filesystem/staging` on first open.
...
and permissions. A marker proves that Ruby owns the directory, and startup only
...
- Ruby records when a file's contents last changed and stores no time chosen by
  ...
  dropped.
- Ruby stores no owner, and every file reports the daemon's own user. Asking for
...
Measured on 2026-08-16 using three fresh `ruby-base_0-8-83` sandboxes, identical
...
| Operation | Two gcsfuse mounts | Ruby FUSE | Ruby/gcsfuse |
| --- | ---: | ---: | ---: |
...
The cross-root gcsfuse cases were five copy/delete fallbacks; all five Ruby
moves were native inode-preserving renames. Eight concurrent 1 MiB writes plus
`fsync` took 9.34 s on gcsfuse and 5.15 s on Ruby. Reading and checking those
eight files took 6.47 s on gcsfuse and 0.25 s on Ruby. Peak filesystem-process
RSS was 212.4 MiB for two gcsfuse processes and 11.5 MiB for one Ruby process.
The Ruby mount also stored and enforced the executable bit. gcsfuse accepted
...
Two Ruby sandboxes mounted the same Pod during the same run. Five overwrites
