...
// to the `ruby-fwd` uid (whose outbound :80/:443 is iptables-REDIRECTed to
// the forwarder) to issue the actual HTTPS request. Denied cases also
// validate the `/tmp/ruby-egress-denied.log` contents.
//
// Commands:
//   matrix     JWT / domain matrix (6 cases) + deny-log validation per deny
//   streaming  Real-world streaming call to a Ruby agent via the forwarder
//   all        matrix + streaming (default)
//
// Required env:
//   EGRESS_PROXY_JWT_SECRET   HS256 secret shared with the proxy
//   RUBY_API_KEY              (streaming only) Ruby workspace API key
//   RUBY_WORKSPACE_ID         (streaming only) target workspace sId
//
// Optional env:
//   EGRESS_PROXY_HOST         default eu.sandbox-egress.ruby.ad
//   EGRESS_PROXY_PORT         default 4443
//   EGRESS_PROXY_TLS_NAME     default = EGRESS_PROXY_HOST
//   EGRESS_PROXY_ALLOWED_DOMAIN default ruby.ad
...
//   DSBX_DENY_LOG             default /tmp/ruby-egress-denied.log
//   RUBY_AGENT_ID             default ruby
//   RUBY_API_BASE_URL         default https://ruby.ad
//   RUBY_AGENT_PROMPT         default a "write a short poem" prompt
...
const PROXY_HOST = process.env.EGRESS_PROXY_HOST ?? "eu.sandbox-egress.ruby.ad";
...
const DENY_LOG_PATH =
  process.env.DSBX_DENY_LOG ?? "/tmp/ruby-egress-denied.log";
const ALLOWED_DOMAIN = process.env.EGRESS_PROXY_ALLOWED_DOMAIN ?? "ruby.ad";
const DENIED_DOMAIN = process.env.EGRESS_PROXY_DENIED_DOMAIN ?? "example.com";
const JWT_ISSUER = "ruby-front";
const JWT_AUDIENCE = "ruby-egress-proxy";
...
async function runAsRubyFwd(
  args: string[]
): Promise<{ code: number; stdout: string; stderr: string }> {
  // `runuser --preserve-environment` keeps RUBY_API_KEY / RUBY_WORKSPACE_ID etc.
  // visible to the child process. Without it, RUBY_* envs get wiped.
  const proc = Bun.spawn({
    cmd: [
      "runuser",
      "-u",
...
async function runMatrixCase(c: MatrixCase): Promise<boolean> {
  ...
  try {
    ...
    const res = await runAsRubyFwd(["fetch", url]);
...
async function cmdStreaming(): Promise<number> {
  ...
  const handle = await startForward();
  try {
    const res = await runAsRubyFwd(["stream"]);
    const ok = res.code === 0;
    if (!ok) {
