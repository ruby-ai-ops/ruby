...
// (see functions-runner/context.ts and @ruby-ai/pod), never applied to
...
const SPAWN_ENV_SCRUB_KEYS = ["RUBY_SANDBOX_TOKEN", "RUBY_POD_USER_IDENTITY"];
 ...
 * shared with dsbx (`~/.ruby-fn/bundles/<sha256>.js`, populated by cold runs
...
export function cachedBundlePath(sha256: string): string | null {
  ...
  const path = join(home, ".ruby-fn", "bundles", `${sha256}.js`);
  try {
