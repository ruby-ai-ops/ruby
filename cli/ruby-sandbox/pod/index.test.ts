...
import type { PodDatabase, SandboxDatabase } from "@ruby-ai/pod";
import {
  db,
  FRAME_ID_ENV,
...
beforeEach(() => {
  databasesDir = mkdtempSync(join(tmpdir(), "ruby-pod-test-"));
...
describe("legacy database compatibility aliases", () => {
  test("legacy environment keys remain exported", () => {
    expect(SANDBOX_DATABASES_DIR_ENV).toBe("RUBY_SANDBOX_DATABASES_DIR");
    expect(POD_DATABASES_DIR_ENV).toBe("RUBY_POD_DATABASES_DIR");
    ...
    expect(POD_DATABASE_PREFIX_ENV).toBe("RUBY_POD_DATABASE_PREFIX");
    expect(SANDBOX_DATABASE_PREFIX_ENV).toBe("RUBY_SANDBOX_DATABASE_PREFIX");
  });
...
describe("must-exist open", () => {
  ...
  test("throws when the databases dir env var is absent", () => {
    ...
    expect(() => db(name)).toThrow(/RUBY_SANDBOX_DATABASES_DIR is not set/);
  });
...
describe("pragmas", () => {
  ...
  test("throws when the quota env var is absent", () => {
    ...
    expect(() => db(name)).toThrow(
      /RUBY_SANDBOX_DATABASE_MAX_SIZE_BYTES is not set/
    );
  });
