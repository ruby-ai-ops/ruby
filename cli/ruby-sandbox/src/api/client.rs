...
const API_URL_ENV: &str = "RUBY_API_URL";

pub struct RubyApiClient {
...
impl RubyApiClient {
    ...
    async fn get<T: DeserializeOwned>(
        ...
        if !status.is_success() {
            ...
            return Err(anyhow::Error::new(RubyApiError::from_http_response(
                status.as_u16(),
                &body,
            ))
            .context(format!("GET {url}")));
    ...
    async fn get_action_status(&self, action_id: &str) -> anyhow::Result<ActionPollResponse> {
        ...
        match parse_action_poll_response(&body) {
            ...
            Err(err) if err.downcast_ref::<RubyApiError>().is_some() => Err(err),
