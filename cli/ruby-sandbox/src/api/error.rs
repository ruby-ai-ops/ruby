...
/// Stable, machine-readable classification of a Ruby API failure.
///
...
/// can `downcast_ref::<RubyApiError>()` — mirroring how transient
/// `reqwest::Error`s are recognized in the poll loop.
#[derive(Debug)]
pub struct RubyApiError {
...
impl fmt::Display for RubyApiError {
...
impl std::error::Error for RubyApiError {}
...
mod tests {
    ...
    fn classifies_invalid_sandbox_token_as_terminal() {
        let err = RubyApiError::from_http_response(
            401,
    ...
    fn classifies_5xx_as_retryable_server_error() {
        let err = RubyApiError::from_http_response(502, "Bad Gateway");
    ...
    fn non_json_body_keeps_raw_message() {
        let err = RubyApiError::from_http_response(400, "<html>nope</html>");
        assert_eq!(err.code, ApiErrorCode::InvalidRequest);
    ...
    fn envelope_json_omits_status_when_unknown() {
        let err = RubyApiError::from_api_error(None, None, "boom");
        let value = err.to_envelope_json();
    ...
    fn display_names_status_and_code() {
        let err = RubyApiError::from_http_response(429, "slow down");
        assert_eq!(err.to_string(), "API error 429 (rate_limited): slow down");
    }

    #[test]
    fn downcasts_through_anyhow_context() {
        let err = anyhow::Error::new(RubyApiError::from_http_response(500, "boom"))
            .context("POST https://ruby.ad/api/v1/w/x/sandbox/actions/call");
        let api = err
            .downcast_ref::<RubyApiError>()
