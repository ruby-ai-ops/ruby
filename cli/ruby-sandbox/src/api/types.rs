...
pub fn parse_action_poll_response(body: &str) -> anyhow::Result<ActionPollResponse> {
    ...
    if let Some(api_error) = super::error::RubyApiError::parse_envelope(None, body) {
...
mod tests {
    ...
    fn parse_error_envelope_bails() {
        ...
        let api_error = err
            .downcast_ref::<super::super::error::RubyApiError>()
            .expect("should carry a RubyApiError");
    ...
    fn call_tool_result_preserves_block_meta_offload_descriptor() {
        ...
        // (key "tt.ruby/offload"); the CLI must pass it through verbatim so in-sandbox
        ...
        let poll_body = r#"{
            ...
            "action":{
                ...
                "output":[{
                    ...
                    "_meta":{"tt.ruby/offload":{"fullContentPath":"pod-x/.tool_outputs/fn/1_tool.json","totalBytes":123456,"contentType":"application/json"}}
                }]
            }
        }"#;
        ...
        let descriptor = &value["content"][0]["_meta"]["tt.ruby/offload"];
        assert_eq!(
