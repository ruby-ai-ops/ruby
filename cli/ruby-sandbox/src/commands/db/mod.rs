...
//!
//! Databases are per-pod SQLite files `{name}.db` under `$RUBY_POD_DATABASES_DIR`
...
pub(crate) const POD_DATABASES_DIR_ENV: &str = "RUBY_POD_DATABASES_DIR";
...
pub enum DbCommand {
    ...
    Reconcile {
        /// Database name (resolved to <name>.db in ${RUBY_POD_DATABASES_DIR})
        name: String,
...
mod tests {
    ...
    // The crate-shared env lock (commands/mod.rs): RUBY_POD_DATABASES_DIR is process-global
