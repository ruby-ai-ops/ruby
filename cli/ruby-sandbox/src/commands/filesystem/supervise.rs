//! Keeps the Ruby FUSE mount running inside a sandbox.
//!
...
fn acquire_supervisor_lock(args: &MountArgs) -> io::Result<File> {
    ...
    let lock_path = mountpoint_parent.join(format!(
        ".{mountpoint_name}.ruby-filesystem-supervisor.lock"
    ));
...
fn detach_mount(mountpoint: &std::path::Path) -> io::Result<()> {
    ...
    if !filesystem_type.starts_with("fuse") || source != "ruby-files" {
...
mod tests {
    ...
    fn mountinfo_identifies_only_the_exact_mountpoint() {
        let mountinfo = concat!(
            "31 20 0:28 / /files-old rw - fuse.ruby-files ruby-files rw\n",
            "32 20 0:29 / /files rw - fuse.ruby-files ruby-files rw\n"
        );

        assert_eq!(
            parse_mountinfo(mountinfo, Path::new("/files")),
            Some(("fuse.ruby-files".to_owned(), "ruby-files".to_owned()))
        );
    ...
    fn mountinfo_decodes_escaped_mountpoint_bytes() {
        let mountinfo = "32 20 0:29 / /run/ruby\\040files rw - fuse.ruby-files ruby-files rw\n";

        assert_eq!(
            parse_mountinfo(mountinfo, Path::new("/run/ruby files")),
            Some(("fuse.ruby-files".to_owned(), "ruby-files".to_owned()))
        );
    }
}
