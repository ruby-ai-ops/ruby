...
const MITM_CA_CERT_PATH: &str = "/run/ruby/egress-ca.pem";
const MITM_CA_KEY_PATH: &str = "/run/ruby/egress-ca.key";

const EGRESS_SECRETS_PATH: &str = "/run/ruby/egress-secrets.json";
...
pub struct ForwardArgs {
    ...
    #[arg(long, default_value = "/tmp/ruby-egress-denied.log")]
    deny_log: PathBuf,
...
pub async fn cmd_forward(args: ForwardArgs) -> Result<()> {
    ...
    // true, reads /run/ruby/egress-ca.pem to build the sandbox trust bundle.
...
fn build_tls_connector_with_alpn(
    ...
    // own CA at /etc/ssl/certs/ruby-egress.pem, so the restarted dsbx will
    ...
    // /run/ruby/egress-ca.key (mode 0600 root-owned). We assume an attacker
