...
const FUNCTIONS_DIR_ENV: &str = "RUBY_FUNCTIONS_DIR";
const FUNCTION_WORKING_DIR_ENV: &str = "RUBY_FUNCTION_WORKING_DIR";
...
pub enum FunctionCommand {
    ...
    Run {
        ...
        /// Function name (resolved to a <name>.<ext> bundle in ${RUBY_FUNCTIONS_DIR})
        name: String,
    },
...
/// instead of inherited, so the caller can deliver it somewhere (e.g. the Ruby
...
/// path in `$RUBY_FUNCTIONS_DIR` (an agent-readable mount), but Bun is launched
...
pub(crate) async fn spawn_function_at(
    ...
    // front sets (e.g. RUBY_POD_DATABASES_DIR, read by `@ruby-ai/pod`) flow
...
/// `$RUBY_FUNCTIONS_DIR`, but keeping the process cwd off the gcsfuse-backed
...
/// Resolve a function name to its bundle file in `$RUBY_FUNCTIONS_DIR`,
...
/// The configured functions directory (`$RUBY_FUNCTIONS_DIR`), required.
...
mod tests {
    ...
    // Run `f` with RUBY_FUNCTIONS_DIR pointing at a fresh temp dir (serialized,
    ...
    /// The pod-databases env contract: front sets it per exec, `@ruby-ai/pod`
    /// reads it in the bun child, and dsbx passes it along by plain env
    /// inheritance — the tests below pin that inheritance.
    const POD_DATABASES_DIR_ENV: &str = "RUBY_POD_DATABASES_DIR";
    ...
    /// the schema text that comes back records what RUBY_POD_DATABASES_DIR
...
export const schema = {
  description: `pod-databases-dir=${process.env.RUBY_POD_DATABASES_DIR ?? "unset"}`,
  input: z.object({}),
  output: z.object({}),
};
export default {
...
"#;
    ...
    /// `@ruby-ai/pod`'s `db()` reads it. spawn_function has no forwarding code —
    ...
    async fn function_get_bun_child_inherits_pod_databases_dir() {
        ...
        // (Empty-string normalization is `@ruby-ai/pod`'s job, tested there.)
