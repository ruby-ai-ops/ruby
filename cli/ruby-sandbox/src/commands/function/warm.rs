...
const WARM_ENABLED_ENV: &str = "RUBY_FUNCTION_WARM_ENABLED";
const SANDBOX_TOKEN_ENV: &str = "RUBY_SANDBOX_TOKEN";
const POD_USER_IDENTITY_ENV: &str = "RUBY_POD_USER_IDENTITY";
...
/// Warm state directory: `$HOME/.ruby-fn`. In the sandbox $HOME is the
...
fn warm_dir() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".ruby-fn"))
}
...
/// under a different RUBY_FUNCTIONS_DIR is a different pod's pool), the
...
fn slot_socket_path(dir: &Path, slot: u32) -> PathBuf {
    let functions_dir = std::env::var("RUBY_FUNCTIONS_DIR").unwrap_or_default();
    dir.join(format!(
...
pub fn spawn_worker(name: &str) {
    ...
    let Ok(functions_dir) = std::env::var("RUBY_FUNCTIONS_DIR") else {
        return;
    };
...
"#
    ...
    // client both read process-global env (HOME, RUBY_FUNCTIONS_DIR). Each
    ...
    async fn warm_cycle_end_to_end() {
        ...
        let original_functions_dir = std::env::var_os("RUBY_FUNCTIONS_DIR");
        ...
        std::env::set_var("RUBY_FUNCTIONS_DIR", bundle_dir.path());
...
"
        ...
        restore_env("RUBY_FUNCTIONS_DIR", original_functions_dir);
    ...
    fn refuses_a_squatted_warm_dir() {
        ...
        let home = tempfile::tempdir().expect("home tempdir");
        std::env::set_var("HOME", home.path());
        let dir = home.path().join(".ruby-fn");
    ...
    fn slot_socket_paths_are_short_stable_and_distinct() {
        let _guard = ENV_LOCK.lock().expect("ENV_LOCK poisoned");
        let dir = PathBuf::from("/home/agent-proxied/.ruby-fn");
    ...
    fn slot_socket_path_distinguishes_functions_dirs() {
        let _guard = ENV_LOCK.lock().expect("ENV_LOCK poisoned");
        let original = std::env::var_os("RUBY_FUNCTIONS_DIR");
        let dir = PathBuf::from("/tmp");

        std::env::set_var("RUBY_FUNCTIONS_DIR", "/mnt/functions/space-a");
        let a = slot_socket_path(&dir, 0);
        std::env::set_var("RUBY_FUNCTIONS_DIR", "/mnt/functions/space-b");
        let b = slot_socket_path(&dir, 0);
        assert_ne!(a, b);

        restore_env("RUBY_FUNCTIONS_DIR", original);
    }
    ...
    fn bundle_cache_refuses_malformed_hashes() {
        ...
        assert!(
            !home.path().join(".ruby-fn/bundles").exists() || {
                std::fs::read_dir(home.path().join(".ruby-fn/bundles"))
