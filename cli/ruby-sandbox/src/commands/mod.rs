...
/// RUBY_POD_DATABASES_DIR). Env vars are shared across the whole test binary, so every module
...
/// also mutates RUBY_POD_DATABASES_DIR) — migrate it to this shared lock after the stacks merge.
#[cfg(test)]
