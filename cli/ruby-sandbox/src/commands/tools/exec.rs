...
use crate::api::{parse_content_block, CallToolResult, ContentBlock, RubyApiClient, RubyApiError};
...
fn error_envelope_json(err: &anyhow::Error) -> serde_json::Value {
    if let Some(api_error) = err.downcast_ref::<RubyApiError>() {
...
mod tests {
    ...
    fn error_envelope_carries_typed_api_error() {
        let err = anyhow::Error::new(RubyApiError::from_http_response(
            403,
