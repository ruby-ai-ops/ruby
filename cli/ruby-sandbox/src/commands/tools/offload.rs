...
// "tt.ruby/offload" key of their `_meta` (front owns the write side in
...
// `cli/ruby-sandbox/pod/tool_output.ts`; both must keep the same resolution
...
/// the wire contract.
const TOOL_OUTPUT_OFFLOAD_META_KEY: &str = "tt.ruby/offload";
...
impl OffloadResolutionError {
    ...
    /// `RubyApiError::to_envelope_json`. No `status`: nothing HTTP failed.
...
mod tests {
    ...
    async fn resolution_keeps_other_meta_keys() {
        ...
        block["_meta"]["tt.ruby/other"] = serde_json::json!({ "k": 1 });
        let resolved = resolve_offloaded_content(&[block], &test_options(mount.path()))
            .await
            .expect("should resolve");

        assert_eq!(resolved[0]["_meta"]["tt.ruby/other"]["k"], 1);
    }
