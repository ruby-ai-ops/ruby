...
#[derive(Parser)]
#[command(name = "dsbx", version, about = "Ruby sandbox CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Version,
    /// Forward sandbox egress traffic to the Ruby egress proxy
...
fn exit_code_for(error: &anyhow::Error) -> i32 {
    if let Some(api_error) = error.downcast_ref::<api::RubyApiError>() {
...
async fn run() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Tools {
            ...
            let client = api::RubyApiClient::from_env()?;
...
mod tests {
    ...
    fn exit_code_classifies_typed_errors() {
        let api_error = anyhow::Error::new(api::RubyApiError::from_http_response(429, "slow down"))
