...
async function main() {
  ...
  const executableBit = await milliseconds(async () => {
    ...
    try {
      ...
      if (result.stdout !== "ruby-executable-ok\n") {
      ...
      // comparison, but require executable bits to work on the Ruby mount.
      if (label === "ruby-database-fuse" || !isAccessDenied(error)) {
        throw error;
      }
    }
