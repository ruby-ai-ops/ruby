#!/usr/bin/env bash
#
# Build the @ruby-ai/pod runtime package and push it to an e2b sandbox.
#
# @ruby-ai/pod is vendored into the sandbox image at build time (see
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo ""
      echo "Build the @ruby-ai/pod package and push it to a running e2b sandbox."
      echo ""
      echo "Options:"
...
if [[ "$NO_BUILD" == false ]]; then
  if ! command -v bun &>/dev/null; then
    echo "Error: 'bun' is not installed (needed to build the @ruby-ai/pod bundle)."
  ...
  echo "==> Building @ruby-ai/pod bundle..."
...
echo "==> Pushing @ruby-ai/pod to sandbox $SANDBOX_ID at $REMOTE_PATH..."
...
echo "==> Done! @ruby-ai/pod deployed to sandbox $SANDBOX_ID"
