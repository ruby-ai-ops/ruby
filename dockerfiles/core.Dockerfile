...
COPY /vendor/rust/eventsource-client/ /app/vendor/rust/eventsource-client/
COPY /core/ .
...
COPY --from=builder /app/core/target/release/check_table /usr/local/bin/check_table
