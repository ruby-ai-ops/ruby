...
# Workspace dep graph: marketing depends only on @ruby-ai/sparkle.
...
# marketing's next build can resolve @ruby-ai/sparkle).
WORKDIR /app/sparkle
COPY /sparkle/ .
RUN npm run build
...
# Build the isolated React 19 workspace demo before test sources are removed.
