...
const SAMPLE_TEMPLATE = {
  ...
  pictureUrl:
    "https://ruby.ad/static/emojis/bg-blue-400/writing_hand/270d-fe0f",
  presetActions: [],
...
vi.mock("@app/lib/api/regions/config", async (importActual) => {
  ...
  return {
    ...
    config: {
      ...mod.config,
      getRubyRegionSyncEnabled: vi.fn(),
      getRubyRegionSyncMasterUrl: vi
        .fn()
        .mockReturnValue("https://main.ruby.ad"),
    },
  };
});
...
describe("POST /api/poke/templates/pull", { sequential: true }, () => {
  ...
  it("returns 400 when called with the sync disabled", async () => {
    vi.mocked(config.getRubyRegionSyncEnabled).mockReturnValue(false);
  ...
  it("handles failed templates list fetch", async () => {
    vi.mocked(config.getRubyRegionSyncEnabled).mockReturnValue(true);
