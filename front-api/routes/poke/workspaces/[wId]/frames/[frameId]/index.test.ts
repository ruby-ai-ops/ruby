...
describe("GET /api/poke/workspaces/:wId/frames/:frameId", () => {
  ...
  it("returns the Frame's share scope and every grant, revoked included", async () => {
    ...
    await frame.addSharingGrantsAndGetCreatedEmails(adminAuth, {
      emails: ["active@ruby.ad", "revoked@ruby.ad"],
    });

    const grants = await frame.listAllSharingGrants();
    const toRevoke = grants.find((grant) => grant.email === "revoked@ruby.ad");
    if (!toRevoke) {
    ...
    expect(revokedAtByEmail.get("active@ruby.ad")).toBeNull();
    expect(revokedAtByEmail.get("revoked@ruby.ad")).toEqual(expect.any(Number));
  });
});
