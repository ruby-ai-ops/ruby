...
import type { AppType, RubyAppRunConfigurationType } from "@app/types/app";
...
export function RubyAppSection() {
  const { owner } = useAgentBuilderContext();
  const { field, fieldState } = useController<
    MCPFormData,
    "configuration.rubyAppConfiguration"
  >({
    name: "configuration.rubyAppConfiguration",
  });
  ...
  const handleRowClick = (app: AppType) => {
    const config: RubyAppRunConfigurationType = {
      id: app.id,
      sId: app.sId,
      appId: app.sId,
          ...
          <div className="flex flex-1 items-center justify-center">
            <div className="px-4 text-center">
              <div className="mb-2 text-lg font-medium text-foreground">
                No Ruby Apps available for this space
              </div>
