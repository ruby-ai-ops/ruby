...
describe("poke tools - security gates", () => {
  describe("get_workspace_metadata", () => {
    ...
    it("denies access from non-Ruby workspace even for super users", async () => {
      const workspace = await WorkspaceFactory.basic();
      const adminAuth = await Authenticator.internalAdminForWorkspace(
        workspace.sId
      );
      ...
      expect(result.isErr()).toBe(true);
      if (result.isErr()) {
        expect(result.error.message).toContain("Ruby-internal workspace");
      }
    });
    ...
    it("denies access when poke_mcp feature flag is disabled", async () => {
      ...
      // Make isRubyWorkspace return true by setting the env var.
      vi.stubEnv("PRODUCTION_RUBY_WORKSPACE_ID", workspace.sId);
    ...
    it("returns workspace metadata when all security gates pass", async () => {
      ...
      // Make isRubyWorkspace return true.
      vi.stubEnv("PRODUCTION_RUBY_WORKSPACE_ID", workspace.sId);
