...
import { isRubyWorkspace } from "@app/types/shared/env";
 ...
 *
 * Gate 1: caller must be a Ruby super user.
 * Gate 2: caller workspace must be the Ruby-internal workspace.
...
export async function enforcePokeSecurityGates(
  ...
  if (!callerUser?.isRubySuperUser) {
    return new Err(
      new MCPError(
        "Access denied: poke tools require Ruby super user privileges."
      )
    );
  }

  if (!isRubyWorkspace(callerWorkspace)) {
    return new Err(
      new MCPError(
        "Access denied: poke tools can only be used from a Ruby-internal workspace."
      )
    );
  }
