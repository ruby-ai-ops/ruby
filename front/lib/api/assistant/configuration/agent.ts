...
> {
  ...
  if (!latestConfig) {
    return new Err(
      new RubyError("internal_error", "Could not find agent configuration")
    );
  }
  ...
  try {
    ...
    if (usersToRemove.length > 0 && agent.scope === "hidden") {
      ...
      if (triggersToDisable.isErr()) {
        return new Err(normalizeAsInternalRubyError(triggersToDisable.error));
      }
    ...
    return new Err(normalizeAsInternalRubyError(error));
  }
}
