...
describe("RubyFileSystem.forConversation", () => {
  beforeEach(() => {
...
describe("RubyFileSystem.forPod", () => {
  it("returns Err(unauthorized) when the user has no membership in the space", async () => {
    ...
    const result = await RubyFileSystem.forPod(regularAuth, projectSpace);
...
describe("RubyFileSystem.forPodSandboxProvisioning", () => {
  it("returns Ok for a same-workspace user who cannot read the pod", async () => {
    ...
    const guarded = await RubyFileSystem.forPod(auth, projectSpace);
    ...
    const result = await RubyFileSystem.forPodSandboxProvisioning(
      auth,
      projectSpace
    );
    assert(result.isOk());
...
describe("RubyFileSystem.forFrameSandboxProvisioning", () => {
  it("builds a GCS filesystem without agent-visible source mounts", async () => {
    ...
    const result = await RubyFileSystem.forFrameSandboxProvisioning(
      auth,
      frame,
      { sandboxOnlyMounts }
    );
...
describe("RubyFileSystem.forUser", () => {
  it("returns Ok with a read-write user mount for the authenticated user", async () => {
    ...
    const result = await RubyFileSystem.forUser(auth);
  ...
  it("returns Err(unauthorized) when there is no authenticated user", async () => {
    ...
    const result = await RubyFileSystem.forUser(noUserAuth);
...
describe("RubyFileSystem.fromScopedPath", () => {
  it("returns Err(invalid_path) for an unrecognised prefix", async () => {
    const { authenticator: auth } = await createResourceTest({});
    const result = await RubyFileSystem.fromScopedPath(
      auth,
  ...
  it("returns Err(invalid_path) for a user- prefix with an empty id", async () => {
    ...
    const result = await RubyFileSystem.fromScopedPath(auth, "user-/memory.md");
...
describe("RubyFileSystem.forAgentLoop", () => {
  ...
  it("matches forConversation mounts when scopedPaths is empty", async () => {
    ...
    const [forConversation, forAgentLoop] = await Promise.all([
      RubyFileSystem.forConversation(auth, conversation),
      RubyFileSystem.forAgentLoop(auth, { conversation }),
    ]);
  ...
  it("throws when scopedPaths references a user-scoped path", async () => {
    ...
    await expect(
      RubyFileSystem.forAgentLoop(auth, {
        conversation,
  ...
  it("adds a second conversation mount when scopedPaths references another accessible conversation", async () => {
    ...
    const result = await RubyFileSystem.forAgentLoop(auth, {
  ...
  it("accepts stat on a dest path in another conversation mount but not with forConversation alone", async () => {
    ...
    const conversationOnlyFs = await RubyFileSystem.forConversation(
      auth,
      currentConversation
    );
    ...
    const agentLoopFs = await RubyFileSystem.forAgentLoop(auth, {
      conversation: currentConversation,
  ...
  describe("access control", () => {
    ...
    it("returns Err(not_found) for another conversation when privateConversationUrlsByDefault is enabled and the user is not a participant", async () => {
      ...
      const result = await RubyFileSystem.forAgentLoop(userSessionAuth, {
    ...
    it("adds a pod conversation mount when the user is a pod member", async () => {
      ...
      const result = await RubyFileSystem.forAgentLoop(refreshedUserAuth, {
        conversation: userConversation,
...
describe("RubyFileSystem.list thumbnail URLs", () => {
  ...
  it("sets thumbnailUrl for image entries in a conversation mount", async () => {
    ...
    expect(file.thumbnailUrl).toBe(
      `https://ruby.ad/api/w/${workspaceId}` +
...
describe("RubyFileSystem.normalizeScopedPath", () => {
  it("returns the path unchanged for a clean canonical path", () => {
    expect(
      RubyFileSystem.normalizeScopedPath("conversation-abc/report.pdf")
  ...
  it("returns null for a path that escapes via ..", () => {
    expect(
      RubyFileSystem.normalizeScopedPath("conversation-abc/../../etc/passwd")
    ).toBeNull();
  });

  it("returns null for a bare .. segment", () => {
    expect(RubyFileSystem.normalizeScopedPath("..")).toBeNull();
  });

  it("returns null for an absolute path", () => {
    expect(RubyFileSystem.normalizeScopedPath("/etc/passwd")).toBeNull();
  });
  ...
  it("returns null when .. escapes a subdirectory but leaves the mount", () => {
    ...
    expect(
      RubyFileSystem.normalizeScopedPath("conversation-abc/../pod-xyz/file.txt")
    ).toBe("pod-xyz/file.txt");
  });
});

describe("RubyFileSystem path traversal rejection (integration)", () => {
  ...
  async function makeFs() {
    ...
    assert(convRes !== null);
    const fs = await RubyFileSystem.forConversation(auth, convRes.toJSON());
    assert(fs.isOk());
    return fs.value;
  }
...
describe("RubyFileSystem legacy path rejection", () => {
  let auth: Authenticator;
  let conversationId: string;
...
describe("RubyFileSystem permission enforcement", () => {
  let auth: Authenticator;
  let conversationId: string;
...
describe("RubyFileSystem.rename", () => {
  let auth: Authenticator;
  let conversationId: string;
  let copyFileMock: ReturnType<typeof vi.fn>;
  let deleteMock: ReturnType<typeof vi.fn>;
...
describe("RubyFileSystem.normalizeScopedPath strips control characters", () => {
  it("strips leading control characters from the filename segment", () => {
    expect(
      RubyFileSystem.normalizeScopedPath("conversation-abc/\n\tsummary.txt")
    ).toBe("conversation-abc/summary.txt");
  });

  it("strips control characters embedded in the mount prefix", () => {
    expect(
      RubyFileSystem.normalizeScopedPath("conversation-abc\x00/notes.txt")
...
describe("RubyFileSystem.toSandboxPath", () => {
  it("resolves a scoped pod path to its absolute sandbox mount path", async () => {
    ...
    const fsResult = await RubyFileSystem.forPod(auth, projectSpace);
    assert(fsResult.isOk());
