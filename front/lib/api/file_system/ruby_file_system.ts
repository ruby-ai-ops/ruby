/**
 * RubyFileSystem is the single entry point for all file system operations in the Ruby platform.
 *
 ...
 *
 * Factories:
 *   RubyFileSystem.forConversation(auth, conversation)   single conversation mount (+pod if project space)
 *   RubyFileSystem.forConversations(auth, conversations) multiple conversation mounts (+pod if project space)
 *   RubyFileSystem.forPod(auth, space)                   single pod mount
 *   RubyFileSystem.forUser(auth)                         the authenticated user's own memory scope
 *   RubyFileSystem.fromScopedPath(auth, scopedPath)     infers context from the path prefix
 *   RubyFileSystem.forAgentLoop(auth, { conversation, scopedPaths })
...
export { RubyFileSystemError } from "@app/types/file_system";
...
): Result<void, RubyFileSystemError> {
...
export class RubyFileSystem {
  ...
  /**
   * Build a RubyFileSystem scoped to one or more conversations.
   *
  ...
  static async forConversations(
  ...
  ): Promise<Result<RubyFileSystem, RubyFileSystemError>> {
    ...
    const backend = RubyFileSystem.createBackend(auth, mounts, storageMode);

    return new Ok(new RubyFileSystem(auth, mounts, backend, storageMode));
  }

  /**
   * Build a RubyFileSystem scoped to a single conversation.
   *
  ...
  static async forConversation(
    ...
    return RubyFileSystem.forConversations(auth, [conversation]);
  }

  /**
   * Build a RubyFileSystem scoped to a pod (project space).
  ...
  static async forPod(
    ...
    const backend = RubyFileSystem.createBackend(
      auth,
      [mount],
      storageMode,
      sandboxOnlyMounts
    ...
    return new Ok(
      new RubyFileSystem(auth, [mount], backend, storageMode, sandboxOnlyMounts)
    );
  }

  /**
   * Build a RubyFileSystem for provisioning a pod's sandbox (mount setup and
  ...
  static async forFrameSandboxProvisioning(
    ...
    return new Ok(
      new RubyFileSystem(auth, [], backend, "gcs", sandboxOnlyMounts)
    );
  }

  /**
   * Build a RubyFileSystem scoped to the authenticated user's own memory space.
   *
  ...
  static async forUser(
    ...
    return new Ok(
      new RubyFileSystem(auth, [createUserMount(user.sId)], backend, "gcs")
    );
  }

  /**
   * Build a RubyFileSystem for an agent loop.
   *
  ...
  static async forAgentLoop(
    ...
    assert(
      userIds.size === 0,
      `User-scoped paths are not supported in the agent loop, access user files through RubyFileSystem.forUser`
    );
    ...
    if (podIdsToFetch.length > 0) {
      ...
      for (const podId of podIdsToFetch) {
        ...
        if (!space) {
          return new Err(
            new RubyFileSystemError("not_found", `Space not found: ${podId}`)
          );
        }
  ...
  /**
   * Build a RubyFileSystem by inferring context from the scoped path prefix.
   *
  ...
  static async fromScopedPath(
    ...
    switch (parsed.kind) {
      case "conversation": {
        ...
        return RubyFileSystem.forConversation(auth, conversation.toJSON());
      }
      ...
      case "pod": {
        ...
        return RubyFileSystem.forPod(auth, space);
      }
      ...
      case "user": {
        ...
        return RubyFileSystem.forUser(auth);
      }
  ...
  private requireReadMount(
    scopedPath: string
  ): Result<{ mount: FileSystemMount; path: string }, RubyFileSystemError> {
    ...
    if (!mount) {
      if (RubyFileSystem.isLegacyPath(normalized)) {
        return new Err(
          new RubyFileSystemError(
            "legacy_path",
  ...
  checkWriteAccess(scopedPath: string): Result<void, RubyFileSystemError> {
    ...
    if (scopedPath !== undefined) {
      ...
      if (resolved.isErr()) {
        logger.warn(
          ...
          "RubyFileSystem.list: access check failed"
        );
        return new Ok([]);
      }
  ...
  async readBuffer(
    scopedPath: string
  ): Promise<Result<Buffer | null, RubyFileSystemError>> {
   ...
   */
  toSandboxPath(scopedPath: string): Result<string, RubyFileSystemError> {
    const resolved = this.requireReadMount(scopedPath);
    if (resolved.isErr()) {
      return resolved;
    }
