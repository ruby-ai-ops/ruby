...
async function resolveFrameFromSource(
  ...
  Result<
    { rubyFs: RubyFileSystem; frame: FileResource; normalizedPath: string },
...
> {
  ...
  return new Ok({ rubyFs, frame, normalizedPath });
}
...
export async function publishFrameFromSource(
  ...
  const { rubyFs, frame, normalizedPath } = resolved.value;
