...
async function getAllMCPServerViewsInWorkspace(
  ...
  Result<MCPServerViewResource[], RubyError<"mcp_server_view_not_found">>
> {
  const views = await MCPServerViewResource.listByMCPServer(auth, mcpServerId);

  if (views.length === 0) {
    return new Err(
      new RubyError("mcp_server_view_not_found", "MCP server views not found")
    );
  }
...
export async function updateOAuthUseCaseForMCPServerViews(
  ...
  Result<undefined, RubyError<"mcp_server_view_not_found" | "unauthorized">>
> {
...
export async function updateNameAndDescriptionForMCPServerViews(
  ...
  Result<
    undefined,
    RubyError<"mcp_server_view_not_found" | "unauthorized" | "name_conflict">
  >
> {
  const r = await getAllMCPServerViewsInWorkspace(auth, mcpServerId);
  if (r.isErr()) {
