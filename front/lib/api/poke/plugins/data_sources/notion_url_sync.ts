...
import { NotionFindUrlResponseSchema } from "@app/types/connectors/admin/cli";
...
export const notionUrlSyncPlugin = createPlugin({
  manifest: {
    ...
    description: "Sync or delete a Notion URL to Ruby",
    resourceTypes: ["data_sources"],
    warning:
      "Sync URLs: Use if a page or database is available on Notion, but not on Ruby, to force the sync.\n" +
      "Delete URLs: Use if a page or database is removed from Notion, but still visible on Ruby.",
    args: {
      operation: {
        type: "enum",
        label: "Operation",
    ...
    async (url: string): Promise<URLOperationResult> => {
      ...
      if (isLeft(decoded)) {
        return {
          ...
          timestamp: Date.now(),
          success: false,
          error: new Error("Unreachable: Invalid response from ruby API"),
        };
      }
