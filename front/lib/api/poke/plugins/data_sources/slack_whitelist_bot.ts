...
function getMetabaseUrl(connectorId: string | null) {
  ...
  return isEU
    ? `https://eu.metabase.ruby.ad/question/46-whitelisted-bots-given-connector?connectorId=${connectorId}`
    : `https://metabase.ruby.ad/question/637-whitelisted-bots-given-connector?connectorId=${connectorId}`;
}
...
export const slackIndexBotMessagesPlugin = createPlugin({
  manifest: {
    ...
    description:
      "Whitelist a Slack bot or workflow so its messages are indexed and searchable in Ruby",
...
export const slackWhitelistBotPlugin = createPlugin({
  manifest: {
    ...
    description:
      "Whitelist a Slack bot or workflow so it can summon ruby agents",
    resourceTypes: ["data_sources"],
    args: {
      botName: {
        type: "string",
