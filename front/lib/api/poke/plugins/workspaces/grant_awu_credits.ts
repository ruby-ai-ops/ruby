  ...
  execute: async (auth, _, args) => {
    ...
    if (isFreeCredit) {
      const result = await createMetronomeCredit({
        ...
        name: `Credits granted by Ruby representative: ${amountCredits.toLocaleString()} credits`,
        idempotencyKey,
    ...
    const commitName = validatedArgs.setPrice
      ? `Commits added by Ruby representative: ${amountCredits.toLocaleString()} credits (manual price)`
      : discountPercent > 0
        ? `Commits added by Ruby representative: ${amountCredits.toLocaleString()} credits (${discountPercent}% discount)`
        : `Commits added by Ruby representative: ${amountCredits.toLocaleString()} credits`;
