...
export const toggleFeatureFlagPlugin = createPlugin({
  ...
  execute: async (auth, _, args) => {
    ...
    const rubyOnlyFeatures = toAdd.filter(
      (feature) => WHITELISTABLE_FEATURES_CONFIG[feature].stage === "ruby_only"
    );
    // On local environments, Ruby-only flags can be enabled freely.
    if (
      !isDevelopment() &&
      rubyOnlyFeatures.length > 0 &&
      (!planCode ||
        (!isRubyCompanyPlan(planCode) && !isFriendsAndFamilyPlan(planCode)))
    ) {
      return new Err(
        new Error(
          "Ruby-only feature flags can only be enabled on Ruby or Friends & Family plans. " +
            `Invalid flags: ${rubyOnlyFeatures.join(", ")}.`
        )
      );
    }
