...
/usr/bin/mkdir -p /etc/ruby "${SYSTEM_CA_CERTS_DIR}/java"

normalized_ca_tmp="$(/usr/bin/mktemp /etc/ruby/.egress-ca.pem.XXXXXX)"
system_tmp=""
bundle_tmp=""
keytool_output=""
cleanup() {
...
# Treat this directory as Ruby-owned staging and keep only the CA we install
...
bundle_tmp="$(/usr/bin/mktemp /etc/ruby/.ca-bundle.pem.XXXXXX)"
