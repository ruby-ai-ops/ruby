...
const RUBY_BASE_IMAGE_VERSION = "0.8.107";
const DSBX_CLI_VERSION = "0.1.57";
const configuredRubyReleasesBaseUrl = EnvironmentConfig.getOptionalEnvVariable(
  "RUBY_RELEASES_BASE_URL"
);
...
const rubyBaseImage = SandboxImage.fromDocker(
  `ruby-sbx-bedrock:${RUBY_BEDROCK_IMAGE_VERSION}`
);

const RUBY_BASE_IMAGE = (
  configuredRubyReleasesBaseUrl
    ? rubyBaseImage.setBuildEnv({
        RUBY_RELEASES_BASE_URL: configuredRubyReleasesBaseUrl,
      })
    : rubyBaseImage
)
  ...
  .runCmd(
    ': "${RUBY_RELEASES_BASE_URL:?RUBY_RELEASES_BASE_URL is required}" && ' +
      `curl -fsSL "\${RUBY_RELEASES_BASE_URL%/}/dsbx-v${DSBX_CLI_VERSION}/dsbx-linux-x86_64" -o /tmp/dsbx && ` +
      `curl -fsSL "\${RUBY_RELEASES_BASE_URL%/}/dsbx-v${DSBX_CLI_VERSION}/checksums-sha256.txt" -o /tmp/checksums-sha256.txt && ` +
  ...
  .runCmd(
    ': "${RUBY_RELEASES_BASE_URL:?RUBY_RELEASES_BASE_URL is required}" && ' +
      `curl -fsSL "\${RUBY_RELEASES_BASE_URL%/}/apply-patch-v${APPLY_PATCH_VERSION}/apply_patch-linux-x86_64" -o /tmp/apply_patch && ` +
      `curl -fsSL "\${RUBY_RELEASES_BASE_URL%/}/apply-patch-v${APPLY_PATCH_VERSION}/checksums-sha256.txt" -o /tmp/checksums-sha256.txt && ` +
