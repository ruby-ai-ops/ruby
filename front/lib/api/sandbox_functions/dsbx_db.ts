...
import type { DbErrorKind } from "../../../../cli/ruby-sandbox/functions-runner/types/db";
...
// (DbCommandError in cli/ruby-sandbox/functions-runner/db/common.ts) and dsbx's own bare
// `{error}` (emit_error in cli/ruby-sandbox/src/commands/function/mod.rs).
...
// Success mirrors the reconcile result in cli/ruby-sandbox/functions-runner/db/reconcile.ts.
...
/**
 * SQLite sidecars that belong to a database file and must go with it. `@ruby-ai/pod` runs with
...
// Success mirrors the `dsbx db list` envelope in cli/ruby-sandbox/src/commands/db/list.rs.
...
const DB_SCHEMA_STAGING_ROOT = "/tmp/ruby-sandbox-db-schemas";
...
// Success mirrors QueryOutcome in cli/ruby-sandbox/functions-runner/db/query.ts.
...
export async function queryDatabaseOnSandbox(
  ...
  const result = await execDbCommand(auth, space, {
    ...
    // set becomes a pod file. Mirrors the pod mount in ruby_file_system.ts. The var name must
    // match POD_QUERY_SPILL_DIR_ENV in cli/ruby-sandbox/src/commands/db/query.rs.
    envVars: {
      RUBY_POD_QUERY_SPILL_DIR: `/files/${SCOPED_PREFIX_POD}${space.sId}/${TOOL_OUTPUTS_FOLDER_NAME}/db`,
    },
    stdin: sql,
  });
  if (result.isErr()) {
