...
const RUBY_REACT_HOOKS_MODULE = "@ruby-ai/react-hooks";
const VIRTUAL_FRAME_ROOT = "/__ruby_frame__";
const VIRTUAL_RUBY_REACT_HOOKS_PATH = `${VIRTUAL_FRAME_ROOT}/node_modules/@ruby-ai/react-hooks/index.d.ts`;
...
function sourceMayCallPodFunction(relPath: string, code: string): boolean {
  ...
  for (const statement of sourceFile.statements) {
    if (
      ...
      statement.moduleSpecifier.text !== RUBY_REACT_HOOKS_MODULE
    ) {
      continue;
    }
...
async function buildRubyReactHooksDeclaration(
...
async function validateCallFunctionTypes({
  ...
  const declaration = await buildRubyReactHooksDeclaration(functionContracts);
  if (declaration.isErr()) {
    return declaration;
  }
  virtualSources.set(VIRTUAL_RUBY_REACT_HOOKS_PATH, declaration.value);
  ...
  const host: ts.CompilerHost = {
    ...
    resolveModuleNames: (moduleNames, containingFile) =>
      moduleNames.map((moduleName) => {
        if (moduleName === RUBY_REACT_HOOKS_MODULE) {
          return {
            resolvedFileName: VIRTUAL_RUBY_REACT_HOOKS_PATH,
  ...
  const rootNames = Array.from(virtualSources.keys()).filter(
    (fileName) => fileName !== VIRTUAL_RUBY_REACT_HOOKS_PATH
  );
  ...
  for (const sourceFile of program.getSourceFiles()) {
    if (sourceFile.fileName === VIRTUAL_RUBY_REACT_HOOKS_PATH) {
      continue;
    }
