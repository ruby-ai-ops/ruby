...
/** Principal used by poke when there is no provisioned Ruby user (e.g. Cloudflare Access). */
...
export class Authenticator {
  ...
  // Set only by poke factory methods (`fromRubySuperUser` / `fromSuperUserSession`).
  // Regular session/API auths keep this false even if the user has the DB flag.
  _isRubySuperUser: boolean;
  // Poke operator principal when no provisioned Ruby user is attached (CF Access).
   ...
   *
   * Only elevates (and sets the poke `_isRubySuperUser` flag) when the session
  ...
  /**
   * Poke operator principal (email/name). Prefers the attached Ruby user when
