...
// stamped with this user's sId (`RUBY_PER_USER_CREDIT_USER`). Two alerts per
 ...
 * the `alert_id` carried in the webhook. Reads the `RUBY_PER_USER_CREDIT_USER`
 ...
 * read from the alert's enforced `RUBY_PER_USER_CREDIT_USER` custom field.
 ...
 * on the credit's `RUBY_PER_USER_CREDIT_USER` custom field so Metronome
...
export async function upsertPerUserCreditBalanceAlerts({
  ...
  const lowBalanceResult = await upsertMetronomeAlert({
    ...
    threshold: Math.floor(allowanceAwu - NEAR_LIMIT_FRACTION * allowanceAwu),
    credit_type_id: creditTypeId,
    customer_id: metronomeCustomerId,
