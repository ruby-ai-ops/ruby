...
describe("seat credit allowances", () => {
  const { contract, productSeatTypes } = buildCachedContractMock({
    seats: [
      { seatType: "free" },
      { seatType: "pro", awu: 500 },
      { seatType: "pro_yearly", awu: 500 },
      { seatType: "max", awu: 2_500 },
      { seatType: "max_yearly", awu: 2_500 },
    ],
  });

  it.each([
    { seatType: "free", credits: 100, period: "lifetime" },
    { seatType: "pro", credits: 500, period: "monthly" },
    { seatType: "pro_yearly", credits: 500, period: "monthly" },
    { seatType: "max", credits: 2_500, period: "monthly" },
    { seatType: "max_yearly", credits: 2_500, period: "monthly" },
  ] satisfies {
    seatType: MembershipSeatType;
    credits: number;
    period: string;
  }[])("displays $credits $period credits for $seatType", ({
    seatType,
    credits,
    period,
  }) => {
    expect(
      getAwuAllocationInfoForSeatType(contract, seatType, productSeatTypes)
    ).toEqual({ credits, period });
  });
});
