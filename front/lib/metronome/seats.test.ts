...
describe("resolveRemappedSeatType — keep / yearly-convert", () => {
  // Contract bills `pro_yearly` (500 AWU) and `max` (2500 AWU).
  const { contract, productSeatTypes } = makeContract({
    seats: [
      { seatType: "pro_yearly", awu: 500, frequency: "ANNUAL" },
      { seatType: "max", awu: 2500, frequency: "MONTHLY" },
    ],
  });
...
describe("resolveRemappedSeatType — none stays none", () => {
  ...
  const { contract, productSeatTypes } = makeContract({
    seats: [{ seatType: "free" }, { seatType: "pro", awu: 500 }],
  });
...
describe("promoteNoneSeatTypesForContract", () => {
  ...
  it("never auto-promotes into a committed max tier", () => {
    const { contract: c, productSeatTypes: pt } = makeContract({
      seats: [{ seatType: "max", awu: 2500, entitled: true }],
    });
    const seatLimits = new Map<MembershipSeatType, SeatLimit>([
  ...
  it("fills the lowest committed tier first when several are available", () => {
    const { contract: c, productSeatTypes: pt } = makeContract({
      seats: [
        { seatType: "pro", awu: 500, entitled: true },
        { seatType: "workspace_yearly", entitled: true },
      ],
    });
    const seatLimits = new Map<MembershipSeatType, SeatLimit>([
...
describe("computeSeatCreditTransfers", () => {
  ...
  it("carries the full allocation when the origin was fully consumed (remaining 0)", () => {
    // 500/500 spent on pro → the whole allocation must carry over so max ends
    // at 500/2500 (remaining 2000), NOT a fresh 2500 on top of the 500
    // already spent (which would let the user spend 3000). A zero balance is a
  ...
  it("carries an overdrawn origin (negative remaining)", () => {
    // Overspent pro by 500 (remaining -500) → consumed is 1000, so max ends at
    // 2500 − 1000 = 1500.
    const transfers = computeSeatCreditTransfers({
      metronomeSeatByUser: new Map([["u1", "pro"]]),
      desiredSeatByUser: new Map([["u1", "max"]]),
  ...
  it("derives consumption from the origin allocation, not the aggregate balance", () => {
    // After a prior pro→max transfer the max seat shows 2494/2500 used 6. The
    // consumed amount must be 6 (2500 − 2494), NOT inflated by an emptied
    // prior tier's starting balance.
    const transfers = computeSeatCreditTransfers({
      metronomeSeatByUser: new Map([["u1", "max"]]),
      desiredSeatByUser: new Map([["u1", "max_yearly"]]),
      balanceByUser: new Map([["u1", 2494]]),
      allocationBySeatType: ALLOCATIONS,
    });
    expect(transfers[0]).toMatchObject({ remaining: 2494, consumed: 6 });
  });

  it("skips moves involving non-recurring-credit seats (workspace, free, none)", () => {
    const balanceByUser = new Map([["u1", 375]]);
