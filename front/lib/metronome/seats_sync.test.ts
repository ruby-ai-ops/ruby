...
describe("syncSeatCount min clamping", () => {
  ...
  it("grants each Free user 100 lifetime credits without granting them twice", async () => {
    mockGetProductSeatTypes.mockResolvedValue(
      new Map([["free-product", "free"]])
    );
    mockGetActiveMemberships.mockResolvedValue({
      memberships: [membership("u1", "free")],
      total: 1,
    });
    ...
    const params = {
      ...
      contract: makeContract([
        { id: "sub_free", productId: "free-product", mode: "SEAT_BASED" },
      ]),
    };

    expect((await syncSeatCount(params)).isOk()).toBe(true);
    expect(mockAddPerUserCredit).toHaveBeenCalledOnce();
    expect(mockAddPerUserCredit).toHaveBeenCalledWith(
      expect.objectContaining({
        amount: 100,
        uniquenessKey: "free-seat-credit:ws_1:u1",
      })
    );
    expect(mockUpsertPerUserCreditAlerts).toHaveBeenCalledWith(
      expect.objectContaining({ allowanceAwu: 100 })
    );

    mockListPerUserCreditUserIds.mockResolvedValue(new Ok(new Set(["u1"])));
    expect((await syncSeatCount(params)).isOk()).toBe(true);
    expect(mockAddPerUserCredit).toHaveBeenCalledOnce();
  });
