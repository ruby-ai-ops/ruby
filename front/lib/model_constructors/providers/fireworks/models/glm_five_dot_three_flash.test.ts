...
const RUBY_CONTEXT_SIZE = 256_000;
const RUBY_MAX_OUTPUT_TOKENS = 64_000;
...
describe("GLM-5.3 Flash model configuration", () => {
  ...
  it("caps context and output in the Ruby layer", () => {
    expect(RubyZAiGlmFiveDotThreeFlashGlobalFireworksStream.contextSize).toBe(
      RUBY_CONTEXT_SIZE
    );
    expect(
      RubyZAiGlmFiveDotThreeFlashGlobalFireworksStream.maxOutputTokens
    ).toBe(RUBY_MAX_OUTPUT_TOKENS);
  ...
  it("maps Ruby's reasoning ladder onto low, high, and max", () => {
    const parseConfig = (config: InputConfig) =>
      RubyZAiGlmFiveDotThreeFlashGlobalFireworksStream.configParsers.reduce(
