  ...
  async ({ step, payload, subscriber }) => {
    await step.email("programmatic-cap-reached-email", async () => {
      ...
      const subject = payload.isBlocked
        ? `[Ruby] Your workspace has reached its programmatic API credit cap in ${payload.workspaceName}`
        : `[Ruby] Your workspace has used 80% of its programmatic API credit cap in ${payload.workspaceName}`;
