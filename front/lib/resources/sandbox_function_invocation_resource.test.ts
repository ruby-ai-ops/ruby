...
describe("SandboxFunctionInvocationResource", () => {
  ...
  it("executes an invocation on the pod sandbox", async () => {
    ...
    expect(opts?.envVars).toMatchObject({
      RUBY_FUNCTIONS_DIR: `/sandbox-functions/pods/${space.sId}`,
      RUBY_POD_DATABASES_DIR: "/sandbox-state/databases",
      RUBY_POD_DATABASE_MAX_SIZE_BYTES: "1073741824",
      // Published outside an app folder, so its databases are unprefixed.
      RUBY_POD_DATABASE_PREFIX: "",
      RUBY_SANDBOX_TOKEN: "sbt-function-token",
      RUBY_FUNCTION_WARM_ENABLED: "0",
    });
    expect(opts?.envVars).not.toHaveProperty(
      "RUBY_FRAME_PUBLICATION_DESCRIPTOR_PATH"
    );
    expect(
      JSON.parse(opts?.envVars?.RUBY_POD_USER_IDENTITY ?? "")
    ).toMatchObject({
    ...
    expect(inputEnvelope).toMatchObject({
      method: "POST",
      url: `https://ruby.local/sandbox-functions/${sandboxFunction.sId}/invocations/${invocation.sId}`,
      headers: {
        "content-type": "application/json",
        "x-ruby-sandbox-function-id": sandboxFunction.sId,
        "x-ruby-sandbox-function-invocation-id": invocation.sId,
      },
  ...
  it("executes a Frame function from its exact immutable publication", async () => {
    ...
    expect(execOptions?.envVars).toMatchObject({
      RUBY_FRAME_PUBLICATION_DESCRIPTOR_PATH: `/frames/${frame.sId}/publications/${publicationId}/publication.json`,
      RUBY_POD_DATABASES_DIR: "/sandbox-state/databases",
      RUBY_POD_DATABASE_MAX_SIZE_BYTES: "1073741824",
      RUBY_POD_DATABASE_PREFIX: "",
      RUBY_FUNCTIONS_DIR: `/frames/${frame.sId}/publications/${publicationId}/functions`,
      RUBY_SANDBOX_TOKEN: "sbt-frame-function-token",
    });
    expect(invocation.gcsPath).toBe(
  ...
  it("reads back a spilled result and delivers the full output", async () => {
    ...
    vi.spyOn(sandbox, "exec").mockResolvedValue(
      new Ok({
        ...
        stdout: stdoutEnvelope({
          ok: true,
          resultFile: "/tmp/ruby-fn-results/spill.json",
          resultBytes: 300_000,
        }),
        stderr: "",
      })
    ...
    expect(readFileSpy).toHaveBeenCalledWith(
      authenticator,
      "/tmp/ruby-fn-results/spill.json"
    );
    const refetched = await SandboxFunctionInvocationResource.fetchById(
      authenticator,
      { sandboxFunction, invocationId: invocation.sId }
  ...
  it("fails the invocation when the spilled result cannot be read back", async () => {
    ...
    expect(refetched?.error).toMatchObject({
      code: "invocation_failed",
      message:
        "Pod function result could not be read back from /tmp/ruby-fn-results/spill.json: file not found",
    });
  });
  ...
  it("passes the app's database prefix, derived from the function slug", async () => {
    ...
    expect(opts?.envVars).toMatchObject({
      RUBY_POD_DATABASE_PREFIX: "task_list__",
    });
  });
...
describe("SandboxFunctionInvocationResource.createAndStartExecution", () => {
  ...
  it("runs a fast invocation inline instead of starting the workflow", async () => {
    ...
    expect(execOptions?.envVars?.RUBY_FUNCTION_WARM_ENABLED).toBe("1");
  ...
  it("runs a durable function under a token that can call tools", async () => {
    ...
    const [, , execOptions] = execSpy.mock.calls[0]!;
    expect(execOptions?.envVars?.RUBY_FUNCTION_WARM_ENABLED).toBe("0");
  });
