 ...
 * passed per exec to `dsbx function run` as `RUBY_POD_DATABASES_DIR`, dsbx forwards it to
 ...
 * `RUBY_POD_DATABASE_MAX_SIZE_BYTES`; both `@ruby-ai/pod`'s `db()` and the `dsbx db query` runner
 ...
 *
 * The `RUBY_POD_*` names are the existing DSBX/@ruby-ai/pod ABI and also apply to Frame-owned state.
 * `databasePrefix` is the Pod app prefix that namespaces the databases the exec resolves by their
 * app-relative name, i.e. `@ruby-ai/pod`'s `db("chat")` inside a published function (see
...
export function sandboxDatabaseExecEnvVars({
  ...
  RUBY_FRAME_PUBLICATION_DESCRIPTOR_PATH?: string;
} {
  return {
    ...
    RUBY_POD_DATABASE_PREFIX: databasePrefix ?? "",
    ...(framePublicationDescriptorPath
      ? {
