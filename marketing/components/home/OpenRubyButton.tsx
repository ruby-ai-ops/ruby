...
import { ArrowRight, LegacyButton as Button } from "@ruby-ai/sparkle";
import { useEffect, useState } from "react";
import { useCookies } from "react-cookie";

interface OpenRubyButtonProps {
...
export function OpenRubyButton({
  variant = "highlight",
  size = "sm",
  trackingArea = TRACKING_AREAS.NAVIGATION,
  trackingObject = "open_ruby",
  showWelcome = false,
}: OpenRubyButtonProps) {
  ...
  if (isLoading) {
    return (
      <Button
        variant={resolveMarketingButtonVariant(variant)}
        size={size}
        label="Open Ruby"
        disabled
        isLoading
