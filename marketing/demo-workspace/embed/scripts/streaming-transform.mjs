...
export function adaptStreamingSource(source, filename, options = {}) {
  ...
  function visit(node) {
    if (
      ...
      node.attributes.properties.some((property) =>
        ...
        property.initializer?.getText(tree) === "{activeScenario}"
      )
    ) {
      matches.conversation += 1;
      edits.push({
        start: node.getStart(tree), end: node.end,
        text: '<StreamConversation key={`${platform}-${activeScenario.id}`} scenario={activeScenario} platform={platform} restored={hasRestored && hasRestoredTheme}>' + node.getText(tree) + '</StreamConversation>',
      });
    }
    if (isPage) {
  ...
  if (
    (isPage &&
      (matches.conversation !== (streamingEnabled ? 1 : 0) ||
  ...
  const streamingImport =
    streamingEnabled && (isPage || isContent)
      ...
      : "";
  return `${streamingImport}${overrideImport}${registryImport}${adapted}`;
}
