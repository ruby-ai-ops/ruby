...
import {
  REMAINING_INDUSTRY_EXPECTED_DOWNLOADS,
  REMAINING_INDUSTRY_INDICATORS,
  REMAINING_INDUSTRY_ORDER,
  REMAINING_INDUSTRY_REQUIRED_TERMS,
  REMAINING_INDUSTRY_TITLES,
  ...
  const scenarioOrder =
    companyId === "meadow"
      ...
      : companyId === "vector"
        ? vectorScenarioOrder
        : null;
  const configuredOrder = REMAINING_INDUSTRY_ORDER[companyId];
  return scenarios
    .map((scenario, index) => ({
      scenario,
      index,
...
export function workspaceScenarioConsistencyIssues(scenario: DemoScenario) {
  const requiredTerms =
    ...
    REMAINING_INDUSTRY_REQUIRED_TERMS[scenario.id];
  if (!requiredTerms) {
    return [];
  }
  ...
  const expectedDownloads =
    ...
    REMAINING_INDUSTRY_EXPECTED_DOWNLOADS[scenario.id];
  if (
    expectedDownloads &&
    JSON.stringify(
