...
import { workspaceChatTitle } from "./scenario-overrides";
...
export function StreamConversation({
  ...
  useLayoutEffect(() => {
    const click = (event: MouseEvent) => {
      ...
      const button = target.closest('button[aria-current="page"]');
      if (
        button?.getAttribute("aria-label") ===
        `${workspaceChatTitle(scenario)} channel`
      ) {
  ...
  useLayoutEffect(() => {
    ...
    function tick() {
      if (disposed || completed || startedAtMs === null || !scroller) {
        return;
      }
      ...
      if (follow && Math.abs(scroller.scrollTop - followTop) > 1) {
    ...
    function start() {
      ...
      followTop = 0;
      shell?.setAttribute("data-stream-state", "playing");
      deadlineId = window.setTimeout(finish, STREAM_DEADLINE_MS);
      tick();
    }
    ...
    const visibility = () => {
      if (
        startedAtMs !== null &&
        performance.now() - startedAtMs >= STREAM_END_MS
      ) {
        finish();
      } else {
        start();
    ...
    try {
      ...
      shell.dataset.streamState = "waiting";
  ...
  }, [scenario, platform, restored, replay]);
