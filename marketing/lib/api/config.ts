...
const config = {
  // Dynamic API base URL: falls back to NEXT_PUBLIC_RUBY_API_URL, with overrides
  ...
  getApiBaseUrl: (): string => {
    ...
    // Remove this when transitioned to RUBY_INTERNAL_API_URL
    override = EnvironmentConfig.getOptionalEnvVariable(
      "RUBY_INTERNAL_CLIENT_FACING_URL"
    );
    if (override) {
      return override;
    }
