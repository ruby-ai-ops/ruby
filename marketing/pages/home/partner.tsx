...
export default function PartnerNextJS() {
  const router = useRouter();

  return (
    <>
      <PageMetadata
        title="Become a Ruby Partner: Join Our Partner Network"
        description="Partner with Ruby to help businesses deploy AI agents. Join our network of resellers, implementation partners, and technology partners."
        pathname={router.asPath}
      />
      <HomeRevealStyles />
      <div className="flex w-full flex-col gap-24 md:gap-32 md:px-12 lg:px-32">
        <PartnerHero />
        {isMarketingSurfaceVisible(MARKETING_SURFACES.partnerWaitlist) && (
          <PartnerIdealPartners />
        )}
      </div>
            ...
            <div className="mx-auto flex w-full max-w-[820px] flex-col items-center gap-8 px-6 text-center">
              ...
              <HomeReveal delay={160} className="max-w-[620px]">
                <p className="m-0 text-base leading-[1.6] text-white/70">
                  Partners are central to our next stage at Ruby. Join the
                  waitlist and we&apos;ll reach out as soon as we&apos;re ready
                  to explore a partnership with you.
                </p>
              </HomeReveal>
              <HomeReveal
                delay={240}
