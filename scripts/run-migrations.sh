...
run_in_region() {
  ...
  local pod_branch
  pod_branch=$(KUBECONFIG="$tmpkubeconfig" kubectl exec "${pod_name}" -- git -C /ruby branch --show-current) || {
    echo "❌ Failed to check /ruby branch in ${region}." >&2
    return 1
  }
  ...
  echo "   → npm run ${NPM_SCRIPT} in /ruby/${COMPONENT}"
