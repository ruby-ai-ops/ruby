Sparkle is Ruby's design system: a published npm package (`@ruby-ai/sparkle`) of React
...
`shapes`, `tag-block`, `dot`, `fire`, `list-add`, `shapes-plus`, `intersect-ruby`, `folder-table`),
...
`./build_icons.sh` deletes `src/icons/v2-stroke` and `src/logo/ruby` outright, clears the
...
stroke icons and the two Ruby mono logos but would leave these marks invisible against their own
