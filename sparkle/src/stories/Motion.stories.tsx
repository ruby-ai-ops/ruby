...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: `Motion tokens as Tailwind utilities. Two decisions: **easing** (entering/exiting → ease-out, moving on screen → ease-in-out) and **duration** (bigger element → longer). Prefer the semantic aliases (\`ease-enter\`, \`duration-enter\`, …). For JavaScript animation libraries, import \`MOTION_EASINGS\` and \`MOTION_DURATIONS\` from \`@ruby-ai/sparkle\`. Each token is shown as a reference row: a continuously looping specimen (a ball tracing the curve, a square spinning at the duration), a click-to-copy class chip, its resolved value, and a description of intended use.`,
      },
    },
  },
} satisfies Meta;
