...
struct AttachmentChipView: View {
        ...
        ZStack(alignment: .topTrailing) {
            content
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10)
                        .stroke(Color.rubyForeground.opacity(0.15), lineWidth: 1)
                )
                .overlay {
            ...
            Button(action: onRemove) {
                Image(systemName: "xmark.circle.fill")
                    .font(.system(size: 18))
                    .foregroundStyle(.white, Color.rubyForeground.opacity(0.7))
            }
            .offset(x: 6, y: -6)
        }
    }
    ...
    private var content: some View {
        if attachment.isImage, let thumbnail = attachment.thumbnailImage {
            ...
            .background(Color.rubyForeground.opacity(0.05))
        }
    }
}
