...
struct TimelineRowView<Content: View>: View {
    ...
    private var iconView: some View {
        ...
        case .circle:
            Circle()
                .strokeBorder(Color.rubyBorder, lineWidth: 1.5)
        ...
        TimelineRowView(icon: .circle, isLast: isLast) {
            HStack(spacing: 4) {
                ...
                if needsTruncation {
                    Image(systemName: "chevron.right")
                        .font(.system(size: 8, weight: .semibold))
                        .foregroundStyle(Color.rubyFaint.opacity(0.5))
...
struct ToolApprovalInlineView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Divider()
                .foregroundStyle(Color.rubyBorder)
        ...
        VStack(spacing: 8) {
            ...
            Button { onValidate?(.approved) } label: {
                Text(canAlwaysAllow ? "Allow once" : "Allow")
                    .sparkleLabelXs()
                    .foregroundStyle(canAlwaysAllow ? Color.rubyForeground : .white)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 12)
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 8) {
                (isSelected ? SparkleIcon.checkCircle : SparkleIcon.circle).image
                    ...
                    .scaledToFit()
                    .frame(width: 16, height: 16)
                    .foregroundStyle(isSelected ? Color.highlight : Color.rubyFaint)
