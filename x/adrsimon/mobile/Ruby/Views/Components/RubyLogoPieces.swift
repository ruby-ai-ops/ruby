...
struct RubyLogoPiece: Identifiable {
    let id: Int
    let color: Color
    let path: (CGRect) -> Path
}

struct RubyLogoPiecesView: View {
    ...
    private static func makePieces() -> [RubyLogoPiece] {
        [
            greenYellowCircle,
            pinkCircle,
            darkGreenRect,
    ...
    private static let greenYellowCircle = RubyLogoPiece(id: 0, color: .green200) { bounds in
    ...
    private static let pinkCircle = RubyLogoPiece(id: 1, color: .rose200) { bounds in
        let scale = bounds.width / 48
        var path = Path()
    ...
    private static let darkGreenRect = RubyLogoPiece(id: 2, color: .green600) { bounds in
        let scale = bounds.width / 48
        var path = Path()
    ...
    private static let redRect = RubyLogoPiece(id: 3, color: .rose500) { bounds in
        let scale = bounds.width / 48
        var path = Path()
    ...
    private static let blueBar = RubyLogoPiece(id: 4, color: .blue500) { bounds in
        let scale = bounds.width / 48
        var path = Path()
    ...
    private static let lightBlueQuarter = RubyLogoPiece(id: 5, color: .blue200) { bounds in
        let scale = bounds.width / 48
        var path = Path()
    ...
    private static let yellowRect = RubyLogoPiece(id: 6, color: .golden500) { bounds in
        let scale = bounds.width / 48
        var path = Path()
