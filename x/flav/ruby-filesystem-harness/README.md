...
```sh
(cd cli/ruby-sandbox/functions-runner && bun install && bun run build)
```
...
```sh
cargo build --manifest-path cli/ruby-sandbox/Cargo.toml --bin dsbx
sudo x/flav/ruby-filesystem-harness/mount_checks.sh
```
