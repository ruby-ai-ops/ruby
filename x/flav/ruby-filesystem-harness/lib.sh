...
DAEMON="${DAEMON:-$REPO_ROOT/cli/ruby-sandbox/target/debug/dsbx}"
MOUNTPOINT="${MOUNTPOINT:-/files}"
STAGING_DIR="${STAGING_DIR:-/run/ruby/staging}"
TOKEN_FILE="${TOKEN_FILE:-/run/ruby/token}"
ACCEPTANCE_SCRIPT="$REPO_ROOT/cli/ruby-sandbox/tests/filesystem_fuse_acceptance.sh"
DAEMON_LOG=/tmp/ruby-filesystem-daemon.log
...
harness_require_daemon() {
  ...
  echo "build one with: cargo build --manifest-path cli/ruby-sandbox/Cargo.toml --bin dsbx" >&2
  exit 1
}

harness_start_front() {
  FRONT_PORT_FILE="$(mktemp)"
  RUBY_FRONT_PORT_FILE="$FRONT_PORT_FILE" python3 "$HARNESS_DIR/fake_front.py" &
  FRONT_PID=$!
