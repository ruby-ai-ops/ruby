...
#
#   x/flav/ruby-filesystem-harness/run.sh mount_checks.sh
#   x/flav/ruby-filesystem-harness/run.sh acceptance.sh
#   x/flav/ruby-filesystem-harness/run.sh staging_checks.sh
#   x/flav/ruby-filesystem-harness/run.sh time_probe.sh
#   x/flav/ruby-filesystem-harness/run.sh space_probe.sh
#
...
SANDBOX_DIR="$REPO_ROOT/cli/ruby-sandbox"
IMAGE=ruby-filesystem-harness
PLATFORM="linux/$(uname -m | sed 's/x86_64/amd64/; s/aarch64/arm64/')"
CARGO_VOLUME=ruby-filesystem-harness-cargo
TARGET_VOLUME=ruby-filesystem-harness-target
PROFILE_FLAG=""
PROFILE_DIR=debug
if [[ -n "${RELEASE:-}" ]]; then
...
if [[ ! -f "$SANDBOX_DIR/functions-runner/runner.js" ]]; then
  ...
  echo "  (cd cli/ruby-sandbox/functions-runner && bun install && bun run build)" >&2
  exit 1
fi
...
docker run --rm \
  ...
  -w /repo \
  "$IMAGE" bash "/repo/x/flav/ruby-filesystem-harness/$SCRIPT"
