# Plug RubyHive into Conductor

Use this when Conductor should create each Git worktree, while RubyHive should
own the per-worktree Ruby environment.

The same pattern works for any external workspace manager:

1. Put the external tool's worktrees under the main Ruby checkout.
2. Run `ruby-hive adopt --path <worktree> --name <env>` after the tool creates a
   worktree.
3. Run `ruby-hive start <env>` to keep the cold environment ready.
4. Install a local Git hook that copies repo-local Claude skills into each
   worktree and points `.codex` at `.claude`.
5. Run `ruby-hive unregister <env>` when the external workspace is archived.
...
`ruby-hive warm <env>` manually only when you need the full app stack.

## 0. Pick your Ruby checkout path

Set this to your main Ruby checkout:

```bash
export RUBY_REPO="$HOME/dev/ruby"
```
...
Install RubyHive from the Ruby checkout:

```bash
cd "$RUBY_REPO/x/henry/ruby-hive"
bun install
bun link
ruby-hive doctor
```
...
Create a Conductor root inside the main Ruby checkout:

```bash
mkdir -p "$RUBY_REPO/.hives/external/conductor"
```
...
4. Set the Conductor root directory to:
   ...
   ```text
   <your Ruby checkout>/.hives/external/conductor
   ```

If the macOS file picker hides `.hives`, press `Cmd+Shift+G` in the picker and
paste the full path.

The root must be under the main Ruby checkout. RubyHive relies on this for the
...
Run this once from the main Ruby checkout:
...
git config core.hooksPath "$RUBY_REPO/.husky"
```
...
Create a machine-local Conductor config in the main Ruby checkout:
...
if HIVE_PATH="$(ruby-hive cd "$CONDUCTOR_WORKSPACE_NAME" 2>/dev/null)"; then
  if [ "$HIVE_PATH" != "$CONDUCTOR_WORKSPACE_PATH" ]; then
    echo "RubyHive env '$CONDUCTOR_WORKSPACE_NAME' already exists at $HIVE_PATH, expected $CONDUCTOR_WORKSPACE_PATH" >&2
    exit 1
  fi

  ruby-hive start "$CONDUCTOR_WORKSPACE_NAME"
else
  ruby-hive adopt \
...
archive = '''
ruby-hive unregister "$CONDUCTOR_WORKSPACE_NAME" || true
'''
...
the cold RubyHive environment. Use the Run button only when you want to call
`ruby-hive start` again.
...
Create a new Conductor workspace for Ruby.

Conductor will create a city-named worktree such as:

```text
<your Ruby checkout>/.hives/external/conductor/workspaces/ruby/kyoto
```
...
The setup script registers that worktree with RubyHive as env `kyoto`.

Check it from any terminal:

```bash
ruby-hive status kyoto
ruby-hive cd kyoto
```
...
the normal `eval "$(direnv hook zsh)"` line has already run and the RubyHive env
does not load.
...
```bash
if ! grep -q 'RUBY_HIVE_CONDUCTOR_WORKSPACES' ~/.zshrc; then
  cat >> ~/.zshrc <<EOF

# RubyHive + Conductor direnv fallback.
export RUBY_HIVE_CONDUCTOR_WORKSPACES="$RUBY_REPO/.hives/external/conductor/workspaces/ruby"
EOF

  cat >> ~/.zshrc <<'EOF'

_ruby_hive_conductor_direnv() {
  emulate -L zsh
  command -v direnv >/dev/null 2>&1 || return 0

  local root="${RUBY_HIVE_CONDUCTOR_WORKSPACES%/}"
...
add-zsh-hook precmd _ruby_hive_conductor_direnv
EOF
fi
```
...
`<worktree>/.envrc` and `~/.ruby-hive/envs/<env-name>/env.sh`.
...
"$RUBY_REPO/.husky/post-checkout"
ruby-hive adopt --path "$workspace" --name "$env_name" --base-branch main
ruby-hive start "$env_name"
direnv allow
```

Do not add `source ~/.ruby-hive/envs/<env-name>/env.sh` to `~/.zshrc`. That
loads one env globally. The hook above lets direnv load and unload the right env
based on the current directory.

## 7. Optional `cn` CLI for nicer names

Conductor creates city-named workspaces. The city directory and RubyHive env
...
metadata="$HOME/.ruby-hive/envs/$hive_env/metadata.json"
if [[ ! -f "$metadata" ]]; then
  echo "Could not find RubyHive metadata for env '$hive_env'." >&2
...
`cn` does not rename the worktree directory or RubyHive env.
...
The city name is already registered in RubyHive. Archive the old Conductor
workspace or unregister the stale env:

```bash
ruby-hive unregister <env-name>
```
...
Remove any `ruby-hive warm` call from Conductor scripts. The scripts should use
only:
