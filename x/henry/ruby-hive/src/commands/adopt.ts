...
export async function adoptCommand(options: AdoptOptions): Promise<Result<void>> {
  ...
  logger.info(`ruby-hive start ${name}         # Ensure cold services are running`);
  logger.info(`ruby-hive warm ${name}          # Only when the full app stack is needed`);
  logger.info(`ruby-hive unregister ${name}    # Remove Hive resources, keep worktree`);
