...
export async function tryCreateTestDatabase(name: string): Promise<void> {
  if (!(await isTestPostgresRunning())) {
    ...
    logger.info("Run 'ruby-hive up' to start it, then 'cd front && npm test' will work");
    return;
  }
...
async function setupWorktree(
  ...
  try {
    ...
    // Non-fatal: continue even if direnv setup fails
  }
...
export async function spawnCommand(options: SpawnOptions): Promise<Result<void>> {
  ...
  if (!currentRepoRoot) {
    return Err(new CommandError("Not in a git repository. Please run from within the Ruby repo."));
  }
  ...
  console.log(`  ruby-hive warm ${name}    # Start all services`);
  console.log(`  ruby-hive open ${name}    # Open terminal session`);
  console.log();
...
function buildOpenOptions(
  ...
  if (options.warm) openOpts.warmCommand = `ruby-hive warm ${name}`;
