 ...
 */
const TMUX_LAYOUT_DIR = join(homedir(), ".ruby-hive", "tmux");
...
export class TmuxAdapter implements MultiplexerAdapter {
  ...
  generateLayout(config: LayoutConfig): string {
    ...
    const lines: string[] = [
      "#!/bin/bash",
      "# Auto-generated tmux layout script for ruby-hive",
      "set -e",
      "",
    ...
    if (unifiedLogs) {
      ...
      lines.push(
        ...
        `tmux send-keys -t "$SESSION_NAME:logs" "ruby-hive logs ${shellQuote(envName)} -i" Enter`,
        ""
      );
    } else {
      ...
      for (const service of ALL_SERVICES) {
        ...
        lines.push(
          ...
          `tmux send-keys -t "$SESSION_NAME:${tabName}" "ruby-hive logs ${shellQuote(envName)} ${shellQuote(service)} -f" Enter`,
          ""
        );
      }
    }
  ...
  generateMainLayout(config: MainLayoutConfig): string {
    ...
    const lines: string[] = [
      "#!/bin/bash",
      "# Auto-generated tmux layout script for ruby-hive main session",
      "set -e",
      "",
      `SESSION_NAME=${shellQuote(sessionName)}`,
      ...
      `tmux send-keys -t "$SESSION_NAME:temporal" "ruby-hive temporal logs" Enter`,
      "",
