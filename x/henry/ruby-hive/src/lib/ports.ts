...
import { RUBY_HIVE_ENVS, RUBY_HIVE_HOME, getPortsPath } from "./paths";
...
const PORT_LOCK_PATH = join(RUBY_HIVE_HOME, "ports.lock");
...
const PortAllocationSchema = z
  ...
  .transform((data) => ({
    ...
    storybook: data.storybook ?? data.base + PORT_OFFSETS.storybook,
...
export function calculatePorts(base: number): PortAllocation {
  const ports: PortAllocation = {
    ...
    storybook: base + PORT_OFFSETS.storybook,
