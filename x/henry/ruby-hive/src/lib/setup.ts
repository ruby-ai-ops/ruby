...
// 1. A small node_modules/@ruby-ai/ override so workspace packages resolve
...
// Setup @ruby-ai workspace overrides in hive's node_modules.
// Since worktrees are under the repo root, Node resolution walks up to find
// {repoRoot}/node_modules/ for all packages. But @ruby-ai/* packages in
...
function setupRubyTtOverrides(repoRoot: string, worktreePath: string): void {
  const mainRubyTt = join(repoRoot, "node_modules", "@ruby-ai");
  const hiveRubyTt = join(worktreePath, "node_modules", "@ruby-ai");

  mkdirSync(hiveRubyTt, { recursive: true });

  for (const pkg of readdirSync(mainRubyTt)) {
    const linkTarget = readlinkSync(join(mainRubyTt, pkg));
    const absoluteTarget = resolve(mainRubyTt, linkTarget);
    const workspaceRelative = absoluteTarget.slice(repoRoot.length + 1);
    symlinkSync(join(worktreePath, workspaceRelative), join(hiveRubyTt, pkg));
  }
}
...
// 1. Override @ruby-ai/* packages to point to the worktree's workspaces
...
export async function installAllDependencies(
  ...
  // Setup @ruby-ai/* overrides (small node_modules/@ruby-ai/ directory).
  // Everything else resolves by walking up to {repoRoot}/node_modules/.
  logger.step("Setting up @ruby-ai workspace overrides...");
  try {
    setupRubyTtOverrides(repoRoot, worktreePath);
    logger.success("@ruby-ai overrides: Linked");
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    logger.error(`@ruby-ai overrides: Failed - ${message}`);
    failed.push("@ruby-ai");
  }
