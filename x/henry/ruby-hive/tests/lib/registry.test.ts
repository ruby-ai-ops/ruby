...
describe("registry", () => {
  describe("SERVICE_REGISTRY", () => {
    ...
    it("proxy config has correct settings", () => {
      ...
      expect(config.cwd).toBe("x/henry/ruby-hive");
  ...
  describe("buildCommand functions", () => {
    ...
    it("connectors uses environment name and port", () => {
      ...
      expect(command).toContain("TEMPORAL_NAMESPACE=ruby-hive-test-connectors");
    ...
    it("connectors uses different port for second environment", () => {
      ...
      expect(command).toContain("TEMPORAL_NAMESPACE=ruby-hive-second-connectors");
